...
namespace Game.Utils.Cache
{
	public class AspNetCache : ICache
	{
		...
		public void Add(Dictionary<string, object> dic)
		{
			...
			{
				HttpContext.Current.Cache.Insert(current.Key, current.Value, null,System.Web.Caching.Cache.NoAbsoluteExpiration, new TimeSpan(0, 20, 0));
			}
		}
		public void Add(Dictionary<string, object> dic, int? timeout)
		{
			...
			foreach (KeyValuePair<string, object> current in dic)
			{
                HttpContext.Current.Cache.Insert(current.Key, current.Value, null, System.Web.Caching.Cache.NoAbsoluteExpiration, new TimeSpan(0, timeout.Value, 0));
			}
		}
		public void Delete()
		{
		...
		public void Update(Dictionary<string, object> dic)
		{
			foreach (KeyValuePair<string, object> current in dic)
			{
				HttpContext.Current.Cache.Insert(current.Key, current.Value);
			}
		}
